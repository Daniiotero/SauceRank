...
export default function UserSearchPage() {
  ...
  useEffect(() => {
    ...
    userApi.getAll()
      ...
      .catch(() => {
        if (active) setError('No se pudieron cargar los usuarios. Inténtalo de nuevo.')
      })
      .finally(() => {
  ...
  return (
    <div className="fade-in">
      ...
      <form
        ...
        style={{ maxWidth: 460, marginBottom: 'var(--space-5)', display: 'flex', gap: 'var(--space-2)' }}
      >
        <input
          value={query}
      ...
      {loading && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxWidth: 460 }}>
          {[0, 1, 2].map(i => (
            <div key={i} className="card skeleton" style={{ height: 70, borderRadius: 'var(--radius-md)' }} />
          ))}
        </div>
      )}

      {!loading && error && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--danger)', fontSize: 14, maxWidth: 460 }}>
      ...
      {!loading && !error && filtered.length === 0 && (
        <EmptyState
          icon="search"
          title={users.length === 0 ? 'Aún no hay usuarios' : 'No se encontraron usuarios'}
          description={users.length === 0 ? 'Cuando alguien se registre aparecerá aquí.' : `No hay resultados para "${query}".`}
          style={{ maxWidth: 460 }}
        />
      )}
